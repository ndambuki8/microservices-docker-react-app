...
class TestUserService(BaseTestCase):
    ...
    def test_add_user(self):
        ...
        with self.client:
            response = self.client.post(
                ...
                data=json.dumps({
                    'username': 'lebron',
                    'email': 'lebronjames@lakers.org'
                }),
                content_type='application/json'
            )
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 201)
            self.assertIn('lebronjames@lakers.org was added!', data['message'])
            self.assertIn('success', data['status'])
    ...
    def test_add_user_invalid_json_keys(self):
        ...
        with self.client:
            response = self.client.post(
                '/users',
                data=json.dumps({'email':'lebronjames@lakers.org'}),
                content_type='application/json',
            )
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_Code, 400)
    ...
    def test_single_user(self):
        ...
        with self.client:
            ...
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 200)
            self.assertIn('lebron', data['data']['username'])
            self.assertIn('lebronjames@lakers.org', data['data']['email'])
            self.assertIn('success', data['status'])
    ...
    def test_main_with_users(self):
        ...
        with self.client:
            ...
            self.assertIn(b'lebron', response.data)
    ...
    def test_main_add_user(self):
        ...
        with self.client:
            response = self.client.post(
                '/',
                data=dict(username='lebron', email='lebronjames@lakers.org'),
